...
**Independent Test**: Đăng nhập, mở **Actions**, rồi xác nhận dữ liệu trong permission_actions được hiển thị và có thể lọc, sắp xếp, chuyển trang mà không cần quyền Actions riêng.
...
- **FR-002**: Mọi người dùng đã đăng nhập MUST có thể xem, tạo, cập nhật và xóa Actions mà không cần kiểm tra quyền Actions.View, Actions.Create, Actions.Update hoặc Actions.Delete; người chưa đăng nhập MUST bị từ chối an toàn.
...
- **FR-007**: Người dùng đã đăng nhập MUST có thể tạo Action với Code và Name.
...
- **FR-011**: Người dùng đã đăng nhập MUST có thể cập nhật Code và Name của Action hiện có.
...
- **FR-020**: Chức năng MUST NOT tạo quyền quản trị mới, gán quyền mới hoặc phụ thuộc vào Permission để quyết định khả năng truy cập và thao tác trên Actions.
...
- **Authenticated administrator**: Người dùng có phiên đăng nhập hợp lệ; không cần quyền Actions riêng để sử dụng chức năng.
...
- **SC-004**: 100% người dùng đã đăng nhập có thể sử dụng đầy đủ chức năng Actions mà không cần quyền Actions riêng; 100% người chưa đăng nhập bị từ chối an toàn.
