...
import { prepareExternalLogin } from "./session";
...
export function LoginPage({ onLoginSuccess }) {
  ...
  useEffect(() => {
    ...
    if (parameters.has("externalError")) {
      setError("Google sign-in could not be completed. Please try again.");
      window.history.replaceState({}, "", window.location.pathname);
    } else if (parameters.has("external")) {
      setError(
        "Google sign-in succeeded, but the local session could not be restored.",
      );
      window.history.replaceState({}, "", window.location.pathname);
    }

    fetch("/backend/external-login/providers")
    ...
    <Box className="cuba-login-page">
      ...
      <Paper className="cuba-login-card" elevation={0}>
        <Stack className="cuba-login-brand" direction="row" spacing={1.25}>
          <Box className="cuba-login-logo">
            <LockRoundedIcon fontSize="small" />
          </Box>
          <Box>
            <Typography className="cuba-login-brand-name">Identity</Typography>
            <Typography className="cuba-login-brand-caption">
              MANAGEMENT
            </Typography>
          </Box>
        </Stack>
        <Box className="cuba-login-heading">
          ...
          <Typography color="text.secondary">
            Enter your code &amp; password to login
          </Typography>
        </Box>
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <Stack spacing={2.25}>
            ...
            <TextField
              ...
              slotProps={{
                htmlInput: { maxLength: 50, pattern: "[A-Za-z0-9._\\-]+" },
              }}
              required
              fullWidth
              autoFocus
        ...
        <Typography className="cuba-login-register" color="text.secondary">
          Don&apos;t have an account?{" "}
          <Link href="#" underline="hover" onClick={showComingSoon}>
            Create Account
          </Link>
        </Typography>
      </Paper>
